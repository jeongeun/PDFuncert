...
void ProdNpGenerator::ProdGenParticle(edm::Event& iEvent, const edm::EventSetup& iSetup) {
   ...
   std::unique_ptr<std::vector<npknu::GenParticle> > genParticleProd ( new std::vector<npknu::GenParticle>() );
   ...
   iEvent.put(std::move(genParticleProd), "GenParticle");
...
void ProdNpGenerator::ProdGenInfo(edm::Event& iEvent, const edm::EventSetup& iSetup) {
   ...
   std::unique_ptr<std::vector<npknu::GenInfo> > genInfoProd ( new std::vector<npknu::GenInfo>() );
   ...
   iEvent.getByToken(genInfoToken, GenEventInfoProducts);
	if(GenEventInfoProducts->binningValues().size()>0){
  	...
  	iEvent.put(std::move(genInfoProd),"GenInfo");
}
