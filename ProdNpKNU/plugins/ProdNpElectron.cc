...
*/
	...
	std::unique_ptr<npknu::ElectronCollection> electronProd ( new npknu::ElectronCollection() );
...
//   for(edm::View<pat::Electron>::const_iterator el = electrons->begin(); el != electrons->end(); el++) {
   for(size_t eleNr=0; eleNr<electrons->size(); eleNr++){
      ...
      ele.nMissingHits            = el->gsfTrack()->hitPattern().numberOfAllHits(reco::HitPattern::MISSING_INNER_HITS);
...
//      ele.effAreaPFIso = pfIso.sumChargedHadronPt + std::max(0.0 , pfIso.sumNeutralHadronEt + pfIso.sumPhotonEt - rhoValue * npknu::EffAreaElectron_Spring15_25ns(el->superCluster()->eta()));
      ...
      //const bool passHEEPV70 = (*heep70Id )[el];
        ele.isCutPassVeto    = el->electronID("cutBasedElectronID-Fall17-94X-V2-tight" );
        ele.isCutPassLoose   = el->electronID("cutBasedElectronID-Fall17-94X-V2-medium");
        ele.isCutPassMedium  = el->electronID("cutBasedElectronID-Fall17-94X-V2-loose" );
        ele.isCutPassTight   = el->electronID("cutBasedElectronID-Fall17-94X-V2-veto"  );
        ele.isCutPassHEEP    = el->electronID("heepElectronID-HEEPV70"                 );//
        ele.passHEEPV70VID   = el->electronID("heepElectronID-HEEPV70"                 );
...
//    ele.CheckVIDvsID = VIDvsID ;
   ...
   iEvent.put(std::move(electronProd),"Electron");
