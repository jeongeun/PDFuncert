...
void ProdNpPhoton::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
   using namespace edm;
	...
	std::unique_ptr<std::vector<npknu::Photon> > photonProd ( new std::vector<npknu::Photon>() );
...
//
		...
		photon.isCutPassTight  = pho->photonID("cutBasedPhotonID-Fall17-94X-V2-tight") ; //(*cutIdDecisionsTight)[phoPtr];
		photon.isCutPassMedium = pho->photonID("cutBasedPhotonID-Fall17-94X-V2-medium") ; //(*cutIdDecisionsTight)[phoPtr];
		photon.isCutPassLoose  = pho->photonID("cutBasedPhotonID-Fall17-94X-V2-loose") ; //(*cutIdDecisionsTight)[phoPtr];
...
//
	...
	iEvent.put(std::move(photonProd),"Photon");
}
