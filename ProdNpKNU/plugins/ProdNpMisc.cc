...
class ProdNpMisc : public edm::EDProducer {
   ...
   private:
      ...
      std::vector<std::string> triggerPaths; bool DoTrigger;
...
ProdNpMisc::ProdNpMisc(const edm::ParameterSet& iConfig) {
   ...
   if(DoVertex and iConfig.existsAs<edm::InputTag>("vertexToken")) {
	...
	if(DoTrigger and iConfig.existsAs<std::vector<std::string > >("triggerPaths")) {
	   triggerBitsToken = consumes<edm::TriggerResults>(iConfig.getParameter<edm::InputTag>("triggerBitsToken"));
	   triggerObjectsToken = consumes<std::vector<pat::TriggerObjectStandAlone> >(iConfig.getParameter<edm::InputTag>("triggerObjectsToken"));
...
void ProdNpMisc::ProdTrigger(edm::Event& iEvent, const edm::EventSetup& iSetup) {
   ...
   std::unique_ptr<std::vector<npknu::Trigger> > triggerProd ( new std::vector<npknu::Trigger>() );
   for (unsigned int j = 0; j < triggerPaths.size(); ++j) {
      std::string idName = triggerPaths[j];
   ...
   iEvent.put(std::move(triggerProd),"Trigger");

   //std::auto_ptr<std::vector<npknu::TriggerObject> > triggerObjectProd ( new std::vector<npknu::TriggerObject>() );
   std::unique_ptr<std::vector<npknu::TriggerObject> > triggerObjectProd ( new std::vector<npknu::TriggerObject>() );
	for(pat::TriggerObjectStandAlone obj : *triggerObjects) { 
	   ...
	   std::vector<std::string> pathNamesLast = obj.pathNames(true);
		...
		//for(unsigned h = 0, n = filterLabels().size(); h < n; ++h) {
        		...
        		//if (obj.hasFilterLabel("hlt") ) bool isBoth =1; 
        		//if (obj.hasFilterLabel("hlt") ) bool isL3   =1; 
        		//if (obj.hasFilterLabel("hlt") ) bool isLF   =1; 
        		//if (obj.hasFilterLabel("hlt") ) bool isNone =1; 
   ...
   iEvent.put(std::move(triggerObjectProd),"TriggerObject");
}
...
void ProdNpMisc::ProdPileup(edm::Event& iEvent, const edm::EventSetup& iSetup) {
   ...
   std::unique_ptr<std::vector<npknu::Pileup> > pileupProd ( new std::vector<npknu::Pileup>() );
   ...
   iEvent.put(std::move(pileupProd),"Pileup");
}
...
void ProdNpMisc::ProdVertex(edm::Event& iEvent, const edm::EventSetup& iSetup) {
   ...
   std::unique_ptr<std::vector<npknu::Vertex> > vertexProd ( new std::vector<npknu::Vertex>() );
   ...
   iEvent.put(std::move(vertexProd),"Vertex");
}
