...
void GetXFX(int nset, std::string pdfsetname, int subset) {
   ...
   for(std::vector<npknu::PDFxfx>::iterator it=pdfXFXvec.begin(); it!=pdfXFXvec.end(); it++) {
      ...
      cout << "--------------------------------------------------" << endl;
      cout << "parton id1 " << it->pdg1 << " x1 " << it->x1 << " Q " << it->scalePDF << " ref_xpdf1 " << it->xpdf1 << endl;
      cout << "parton id2 " << it->pdg2 << " x2 " << it->x2 << " Q " << it->scalePDF << " ref_xpdf2 " << it->xpdf2 << endl;
      xfxPair.first  = pdf->xfxQ(it->pdg1, it->x1, it->scalePDF);
      xfxPair.second = pdf->xfxQ(it->pdg2, it->x2, it->scalePDF);
      cout << "lhapdf " << pdfsetname << "_" << subset << "_xpdf1 " << xfxPair.first << endl;
      cout << "lhapdf " << pdfsetname << "_" << subset << "_xpdf2 " << xfxPair.second << endl;
